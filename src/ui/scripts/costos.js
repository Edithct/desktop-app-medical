...
let tableManager = new TableManager(
    'SELECT cat.nombre as categoria ,se.* FROM `servicios` se left join servicios cat on se.categoria_id=cat.id WHERE se.es_servicio=1 and se.es_ingreso=0 and se.deleted_at is null and cat.deleted_at is null order by cat.nombre asc, se.nombre asc limit $1 offset $2;',
    main.repository
)
...
let selectCategory=new TableManager('select * from servicios where deleted_at is null and es_servicio=0 and es_ingreso=0 order by nombre asc;',main.repository);
